...
export function calculateKarma(input: KarmaInput): number {
  switch (input.action) {
    ...
    case 'VERIFY':
      return 5
    case 'RESOLVE':
      if (input.timeTaken != null && input.slaHours != null) {
...
export function getKarmaLevel(totalKarma: number): { level: string; nextLevel: number; progress: number } {
  const levels = [
  ...
  const currentLevel = levels.slice().reverse().find(l => totalKarma >= l.threshold) || levels[0]
  const nextLevel = levels.find(l => l.threshold > totalKarma)
  return {
    level: currentLevel.name,
    nextLevel: nextLevel?.threshold ?? currentLevel.threshold,
    progress: nextLevel ? ((totalKarma - currentLevel.threshold) / (nextLevel.threshold - currentLevel.threshold)) * 100 : 100
  }
}
