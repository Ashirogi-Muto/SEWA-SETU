...
graph TB
    ...
    subgraph "AI Services"
        AI_Server[AI Model Server<br/>YOLOv8 Local]
        Sarvam[Sarvam API<br/>Speech-to-Text]
    end
    
    subgraph "Data Layer"
        DB[(SQLite<br/>Report Data)]
    end
    
    CP -->|HTTPS/REST| API
    AD -->|HTTPS/REST| API
    API -->|Image Classification| AI_Server
    API -->|Audio Transcription| AI_Server
    AI_Server -->|STT| Sarvam
    API -->|Read/Write| DB
```
...
sequenceDiagram
    ...
    participant AI as AI Model Server (Local)
    participant Sarvam as Sarvam STT API
    participant DB as SQLite Database
    
    C->>API: Submit Report (text/voice/images)
    
    alt Has Images
        API->>AI: Send image for classification (YOLOv8)
        AI-->>API: Return category & confidence
    end
    
    alt Has Voice
        API->>AI: Send audio for transcription
        AI->>Sarvam: Route audio to Sarvam
        Sarvam-->>AI: Return text
        AI-->>API: Return transcription
    end
