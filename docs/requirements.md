...
### Requirement 2: Hinglish Language Processing & Automated Categorization

**User Story:** As an administrator, I want reports to be automatically categorized and the NLP layer to understand Hinglish, so that reports can be routed efficiently even when written in a mix of Hindi and English.

#### Acceptance Criteria

1. WHEN text input contains Hinglish content, THE System SHALL process and understand the language input using AI.
2. WHEN the NLP service processes the report, THE System SHALL assign a Category.
3. THE System SHALL support categories including roads, sanitation, water_supply, electricity, street_lighting, parks, and other.
4. WHEN the NLP service assigns a category with low confidence, THE System SHALL flag the report for manual categorization.
5. THE System SHALL allow administrators to override automatic categorization.

### Requirement 3: Voice Transcription Support
...
1. WHEN a citizen records voice input, THE Citizen_Portal SHALL capture audio in a compatible format.
2. WHEN the audio is uploaded, THE System SHALL send it to the Transcription service.
3. WHEN the Transcription service completes processing, THE System SHALL store the transcribed text with the report.
4. THE Transcription service SHALL support Hindi and English transcription.
5. WHEN transcription fails, THE System SHALL retain the original audio file and notify the citizen.
